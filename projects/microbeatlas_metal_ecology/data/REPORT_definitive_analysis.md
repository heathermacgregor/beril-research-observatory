...
|---------|---------|---------------|---------|---------|
| CWM (this analysis) | CWM ~ measured USGS metal, lm(); full 28-covariate model | 75 | — | — |
| **SPIRE NB16 (full covariates — symmetric)** | KO binary ~ measured USGS metal; Z = lat/lon, pH, drainage, lith, mine_dist, clay, OM, lc_*, phylum | **832** | **0** | **2** |
| SPIRE NB14 (lat+phylum only — asymmetric calibration) | KO binary ~ measured USGS metal; Z = lat + phylum only | 476 | 2 | 7 |
| SPIRE NB04 (PF1 modeled — invalid comparison) | KO binary ~ Qi et al. 2025 PF1 predictions | 56 | 0 | 0 |

**NB16 comparison (symmetric — same metal source, same full covariate set):** NB16 runs the vectorized OLS score test on SPIRE MAG binary KO presence/absence against USGS measured metals (36,691 total tests), after nearest-neighbour-joining each MAG to the 634 thinned USA cells and residualizing the same covariate set used for CWM: lat, lon, pH (SoilGrids), drainage class (dummies), GLiM lithology (dummies), mine distance, clay %, organic matter, land-cover fractions (forest/cultivated/urban), and the MAG's own phylum. **Zero exact KO×metal overlaps** with the 75 CWM hits. KO-level overlap: 2 (K14153, K21898), against a null expectation of ~1.7 pairs — within chance. This is the valid symmetric comparison.

**NB14 confounding check:** Of NB14's 476 hits (lat+phylum-only SPIRE), only 14/476 (3%) survive when the full covariate set is applied (NB16). The remaining 462 NB14 hits were confounded by soil properties that NB14 did not control. NB16 finds a largely different set of 832 significant KOs, none of which overlap with the CWM 75 hits.

**Interpretation:** With symmetric full confounding applied to both SPIRE and CWM, the two analyses share 0 exact gene×metal associations (2 KO-level at null expectation). CWM captures *community turnover* — which genera dominate at high-metal sites, weighted by their KO content (16S inferred). SPIRE captures *gene-level selection within the MAG pool* — whether individual MAGs carrying a given KO are enriched at high-metal sites (metagenomic). Near-zero overlap is the expected signature of a turnover-dominant mechanism: community composition shifts radically via differential genus abundances (strong CWM signal) without individual lineages gaining or losing the genes that define those genera (no SPIRE signal). Consistent with Adam's reframing.
